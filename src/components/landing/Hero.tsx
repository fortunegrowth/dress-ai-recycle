      ...
      <div className="relative z-10 mx-auto flex min-h-[100svh] max-w-6xl flex-col justify-center px-5 py-24 sm:px-8">
        <div className="grid grid-cols-[1.3fr_1fr] items-center gap-4 sm:gap-8 lg:grid-cols-[1.1fr_1fr] lg:gap-10">
          <div className="reveal">
            ...
            <h1 className="mt-5 sm:mt-7 font-serif text-[1.5rem] leading-[1.35] sm:text-6xl sm:leading-[1.2] text-balance">
              売るかどうか、<br />
              <span className="text-gold">今日中</span>に決められる。
            </h1>

            <p className="mt-4 sm:mt-6 max-w-xl text-xs sm:text-lg leading-relaxed text-primary-foreground/90">
              写真を送るだけで30秒。<br />
              店舗にも行かず、数日も待たず、<br />
              家にいながら適正価格がわかります。
            </p>
            ...
            <div className="mt-6 sm:mt-9 flex flex-col sm:flex-row gap-3">
              <a
                href="#cta"
                className="inline-flex items-center justify-center rounded-full bg-gold px-5 sm:px-7 py-3 sm:py-4 text-xs sm:text-sm font-medium text-gold-foreground shadow-xl shadow-black/30 transition hover:opacity-90"
              >
                無料で査定を申し込む
              </a>
              <a
                href="#how"
                className="inline-flex items-center justify-center rounded-full border border-primary-foreground/40 bg-primary-foreground/10 px-5 sm:px-7 py-3 sm:py-4 text-xs sm:text-sm font-medium text-primary-foreground backdrop-blur transition hover:bg-primary-foreground/20"
              >
                仕組みを見る
              </a>
            </div>
