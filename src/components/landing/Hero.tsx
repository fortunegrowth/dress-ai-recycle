  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto flex min-h-[100svh] max-w-6xl flex-col justify-center px-5 py-24 sm:px-8">
        <div className="grid grid-cols-[1.15fr_1fr] items-center gap-4 sm:gap-8 lg:grid-cols-[1.1fr_1fr] lg:gap-10">
          <div>
            <div className="flex flex-col items-start gap-1.5 sm:flex-row sm:flex-wrap sm:gap-2">
              <span className="inline-flex items-center gap-1 sm:gap-1.5 rounded-full border border-gold/60 bg-primary/50 px-2.5 sm:px-3 py-1 text-[10px] sm:text-[11px] font-medium backdrop-blur">
                <Sparkles className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-gold" />
                30秒査定
              </span>
              <span className="inline-flex items-center rounded-full border border-primary-foreground/30 bg-primary/50 px-2.5 sm:px-3 py-1 text-[10px] sm:text-[11px] backdrop-blur">
                着用回数が少ないほど高価買取
              </span>
            </div>

            <h1 className="mt-4 sm:mt-6 font-serif text-[1.5rem] leading-[1.3] sm:text-6xl sm:leading-[1.2] text-balance">
              売るかどうか、<br />
              <span className="text-gold">今日中</span>に<br className="sm:hidden" />決められる。
            </h1>

            <p className="mt-3 sm:mt-5 text-[11px] sm:text-lg leading-relaxed text-primary-foreground/90">
              写真を送るだけで30秒。<br />
              店舗にも行かず、数日も待たず、<br />
              家にいながら適正価格がわかります。
            </p>
            ...
            <div className="mt-5 sm:mt-7 flex flex-col sm:flex-row gap-2.5 sm:gap-3">
              <a
                href="#cta"
                className="inline-flex items-center justify-center rounded-full bg-gold px-4 sm:px-6 py-3 sm:py-3.5 text-xs sm:text-sm font-medium text-gold-foreground shadow-xl shadow-black/30 transition hover:opacity-90"
              >
                無料で査定を申し込む
              </a>
              <a
                href="#how"
                className="inline-flex items-center justify-center rounded-full border border-primary-foreground/40 bg-primary-foreground/10 px-4 sm:px-6 py-3 sm:py-3.5 text-xs sm:text-sm font-medium text-primary-foreground backdrop-blur transition hover:bg-primary-foreground/20"
              >
                仕組みを見る
              </a>
            </div>
